function Terminal(container, state) {
  ...
  function moveLeft(remove) {
    if (!caret.previousSibling || caret.previousSibling.textContent == "") {
      return false;
    }

    if (!remove) {
      if (!caret.nextSibling) {
        caret.parentNode.appendChild(document.createTextNode(""));
      }
  ...
  function moveRight(remove) {
    if (!caret.nextSibling || caret.nextSibling.textContent == "") {
      return false;
    }

    if (!remove) {
      if (!caret.previousSibling) {
        caret.parentNode.insertBefore(document.createTextNode(""), caret);
      }
  ...
  function write(event) {
    if (alive && event.which >= 32 && !event.ctrlKey) {
      if (!caret.previousSibling) {
  ...
  function execute() {
    ...
    if (caret.parentNode.textContent !== " ") {
  ...
  window.addEventListener("click", function(e) { 
    caret.classList.toggle("nofocus", !(alive = container.contains(e.target)));
  }, true);
